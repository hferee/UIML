...
\end{definition}
Note that a right uniform interpolant is unique up to equivalence, and so is a left uniform interpolant. In previous work~\cite{FerGoo2023} the first- and second-named author developed a Coq/Rocq formalization of the construction of uniform interpolants for intuitionistic propositional logic, which was extended in~\cite{FGGS2024} to modal logics $\mathbf{K}$ and $\mathbf{GL}$, and to intuitionistic strong Löb logic $\mathbf{iSL}$.
