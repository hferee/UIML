\documentclass[english,final]{jflart}
...
%
\begin{definition}[\coqdoc{ISL.Formulas.html\#weight}]\label{dfn:weight}
The \emph{weight of a formula} (\coqdoc{ISL.Formulas.html\#weight}) is inductively defined as:
...
\begin{itemize}
    ...
    \item not invertible: {${\vee \mathrm R}_1, {\vee \mathrm R}_2$} and {$\rightarrow\rightarrow$L}.
\end{itemize}
...
\begin{definition}[Uniform interpolants]\label{dfn:prop-quantifiers}
	Let $\phi$ be a formula and $p$ a propositional variable. A formula $\exists p. \phi$ is a \textit{right uniform interpolant} for $\phi$ with respect to $p$ if it is a $p$-free formula such that $\phi \vdash \exists p. \phi$, and for any formula $\psi$, if  $\phi \vdash \psi$ then $\exists p.\phi \vdash \psi$, i.e. $\exists p. \phi$ is the strongest $p$-free formula that is entailed by $\phi$.

	Dually, $\forall p. \phi$ is a \textit{left uniform interpolant} for $\phi$ with respect to $p$ if it is a $p$-free formula such that $\forall p. \phi \vdash \phi$, and for any formula $\theta$, if $\theta \vdash \phi$ then $\theta \vdash \forall p.\phi$, i.e. $\forall p. \phi$ is the weakest $p$-free formula that entails $\phi$.
\end{definition}
Note that a right uniform interpolant is unique up to equivalence, and so is a left uniform interpolant. In previous work~\cite{FerGoo2023} the first- and second-named authors developed a Coq/Rocq formalization of the construction of uniform interpolants for intuitionistic propositional logic, which was extended in~\cite{FGGS2024} to modal logics $\mathbf{K}$ and $\mathbf{GL}$, and to intuitionistic strong Löb logic $\mathbf{iSL}$.
...
Indeed, the inductive definition given by Figure~\ref{fig:iseq-pc} does not guarantee decidability by construction.
...
\begin{algorithmic}[lines]
	...
	$\delta \cons \Delta_1 \cons \Delta_2 \cons \Delta_3 \cons p \vdash_? \phi$
...
\end{equation}
After two recursive calls, we will try to simplify $p \to q$. At this point, we should remember that we are simplifying the formula $p \to q$ in a context that contains $p$. Thus, our recursive procedure will have to keep track of the \emph{context} in which we are simplifying as an additional parameter. This leads us to define a \emph{contextual simplification} function, called \texttt{contextual\_simp\_form} in our code (\coqdoc{ISL.Simp_env.html\#S.contextual_simp_form}), and \texttt{csimp} for short here. We give the scheme of the definition in~\cref{def:c-simp} below. The simplification on formulas, \texttt{simp\_form}, will then be defined as \texttt{csimp} in an empty context.
...
\begin{enumerate}
    ...
    \item \label{simp2}Check whether there is $\phi \in \Delta$ such that $\Delta \setminus \{ \phi \} \vdash \phi$; if so, continue with $\Delta' := \Delta \setminus \{\phi\}$.
    \item \label{simp3}Finally, check whether there is a formula $\phi$ in $\Delta$ to which a {contextual simplification} applies, and if so, obtain $\Delta'$ by replacing $\phi$ with $\csimp \; (\Delta \setminus \{\phi\}) \; \phi$.
\end{enumerate} 
The second rule consists in weakening a redundant hypothesis, whereas the third one more generally simplifies
each formula using the remainder of the context. The latter is however computationally more costly, which is why
redundant formulas are removed first.
