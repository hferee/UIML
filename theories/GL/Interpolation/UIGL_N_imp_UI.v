  ...
  - assert ((Canopy (nodupseq s)) = [nodupseq s]). apply Id_InT_Canopy ; auto.
    ...
    * rewrite H. simpl. unfold N.
      ...
      +  rewrite ub_nodupseq in f0. pose (GN_inv_noinit_nolessub _ g J40 f0 J41). rewrite <- e. clear e.
          ...
          (* LHS s is empty *)
          -- assert (GUI p s (Or (list_disj (restr_list_prop p (snd s))) (list_disj (map Box (map (UI p) (GLR_prems (nodupseq s))))))).
              apply GUI_critic_not_init_emptyLHS ; auto. apply (UI_GUI p s) in H2. rewrite H2. simpl.
              ...
              remember (Or (list_disj (restr_list_prop p (snd s))) (list_disj (map Box (map (UI p) (GLR_prems (nodupseq s)))))) as disj2.
              ...
              epose (OrL (_,_)). simpl in g2. apply g2 ; clear g2.
              rewrite <- HeqLHS ; simpl. apply derI with (ps:=[]). apply BotL. apply (BotLRule_I [] []). apply dlNil.
              apply GLS_prv_wkn_R with
              (s:= ([list_disj (map Box (map (UI p) (GLR_prems (nodupseq (nodupseq s)))))], [(list_disj (map Box (map (UI p) (GLR_prems (nodupseq s)))))]))
              (A:=list_disj (restr_list_prop p (snd s))).
              2: epose (wkn_RI (list_disj (restr_list_prop p (snd s))) _ [] _) ; simpl in w ; apply w.
              eapply (list_disj_L) with (s:=([],_)). intros. simpl.
              apply InT_map_iff in H3. destruct H3. destruct p0 ; subst. apply InT_map_iff in i.
              destruct i. destruct p0 ; subst.
              apply nodupseq_GLR_prems in i. destruct i. destruct p0. destruct p0. subst.
              epose (list_disj_wkn_R _ (_,[]) (Box (UI p x0))). apply g2 ; clear g2. apply InT_map_iff.
              ...
              apply GLR. epose (@GLRRule_I _ [Box (UI p x1)] _ [] []).  simpl in g2. apply g2.
              intros A H3. inversion H3 ; subst. eexists ; auto. inversion H4. apply univ_gen_ext_refl.
          ...
          (* LHS s is not empty *)
          -- assert (J50: fst s <> []). intro. rewrite <- HeqLHS in H2. inversion H2. rewrite HeqLHS in *.
              pose (@GUI_inv_critic_not_init_not_emptyLHS p s _ _ _ J0 c J50 H0 J1 J42). rewrite <- e. clear e. simpl.
              remember (Or (list_disj (restr_list_prop p (nodup eq_dec_form (snd s)))) (Or (list_disj (map Neg (restr_list_prop p (nodup eq_dec_form (fst s))))) (list_disj (map Box (map (UI p) (GLR_prems (nodupseq (nodupseq s)))))))) as disj1.
