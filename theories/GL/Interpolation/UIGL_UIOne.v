...
Require Import UIGL_UI_prelims.
  ...
  pose (LexSeq_ind (fun s => forall (p q : nat), In (Var q) (propvar_subform (UI p (fst s, snd s))) ->
                                                      ((q <> p) * (In (Var q) (propvar_subform_list (fst s ++ snd s)))))).
  apply p. clear p.
  intros. rewrite propvar_subform_list_app. destruct (critical_Seq_dec s0).
  ...
  - destruct (dec_init_rules s0).
    ...
    + remember (fst s0) as LHS. destruct LHS.
      ...
      -- assert (GUI p s0 (Or (list_disj (restr_list_prop p (snd s0))) (list_disj (map Box (map (UI p) (GLR_prems (nodupseq s0))))))).
         ...
         destruct s0. rewrite HeqLHS. simpl. simpl in H1. simpl in H0. apply UI_GUI in H1. rewrite HeqLHS in H0 ; simpl in H0. rewrite H1 in H0. simpl in H0.
         ...
         apply in_or_app ; right ; apply in_or_app ; auto. simpl in H0. rewrite <- app_nil_end in H0.
         apply in_or_app ; right. apply propvar_subform_list_nodup ; rewrite <- H4 ; repeat rewrite propvar_subform_list_app ; 
      ...
      -- assert (GUI p s0
         ...
         apply GUI_critic_not_init ; auto. intro. rewrite <- HeqLHS in H1. inversion H1.
         1-2: apply Gimap_map ; intros. apply UI_GUI ; auto.
         apply (@N_spec p s0 x). apply UI_GUI in H1. destruct s0. rewrite HeqLHS in H0 ; simpl in H0. rewrite H1 in H0. simpl in H0.
         repeat rewrite <- app_nil_end in H0. clear H1. rewrite HeqLHS ; simpl.
         apply in_app_or in H0. destruct H0.
         ...
         destruct (eq_dec_nat q p). exfalso ; apply H2 ; subst ; auto. split ; auto. apply in_or_app ; left. simpl in HeqLHS.
         ...
         apply propvar_subform_list_Canopy with (A:=# q) in H2 ; auto. simpl in H2. rewrite <- app_nil_end in H2.
         ...
         rewrite <- e in H1. clear e. clear J1. clear H0. simpl in H1. repeat rewrite <- app_nil_end in H1. apply in_app_or in H1.
         ...
         repeat rewrite propvar_subform_list_app in i0. simpl in i0. repeat rewrite <- app_nil_end in i0.
