...
Require Import UIGL_UI_prelims.
  ...
  - pose (PermutationTS_critic _ _ perm c).
    ...
    * assert ((is_init s) -> False) ; auto. assert ((is_init sp) -> False). intro. apply H.
      ...
      (* LHS is empty. *)
      + assert (J40: fst s = []). rewrite <- HeqLHS ; auto. assert (J41: fst sp = []). destruct perm.
         destruct (Permutation_PermutationT (fst s) (fst sp)). pose (p3 p0). rewrite J40 in p4. apply Permutation_nil ; auto.
          pose (@GUI_inv_critic_not_init_emptyLHS p s _ _ J0 c J40 H J1). rewrite <- e ; clear e.
          pose (@GUI_inv_critic_not_init_emptyLHS p sp _ _ J00 c0 J41 H0 J10). rewrite <- e ; clear e.
          ...
          pose (@GLS_prv_wkn_R ([list_disj (map Box (map (UI p) (GLR_prems (nodupseq s))))], [list_disj (map Box (map (UI p) (GLR_prems (nodupseq sp))))])).
          apply g with (A:=list_disj (restr_list_prop p (snd sp))). clear g.
          ...
          epose (list_disj_wkn_R _ ([_],[]) (Box (UI p x))). simpl in g. apply g ; clear g.
      ...
      (* LHS is not empty. *)
      + rewrite HeqLHS in *. assert (J40: fst s <> []). rewrite <- HeqLHS ; intro Hn ; inversion Hn. assert (J41: fst sp <> []). destruct perm.
         destruct (Permutation_PermutationT (fst s) (fst sp)). pose (p3 p0). intro. apply J40. rewrite H1 in p4.
         apply Permutation_sym in p4 ; apply Permutation_nil ; auto.
         pose (@GUI_inv_critic_not_init_not_emptyLHS p s _ _ _ J0 c J40 H J1 J2). rewrite <- e ; clear e.
         pose (@GUI_inv_critic_not_init_not_emptyLHS p sp _ _ _ J00 c0 J41 H0 J10 J20). rewrite <- e ; clear e.
          epose (OrR (_,[])). simpl in g. apply g ; clear g.
          epose (OrL ([],_)). simpl in g. apply g ; clear g.
          epose (list_disj_L _ (_,_)). apply g ; clear g. simpl ; intros.
          pose (PermutationTS_restr_list_prop_snd _ _ _ _ perm H1).
  ...
  - assert (critical_Seq (nodupseq s)). apply (critical_nodupseq s) ; auto.
    ...
    * assert ((is_init s) -> False) ; auto. assert ((is_init (nodupseq s)) -> False). intro. apply H0.
      ...
      remember (fst s) as LHS. destruct LHS.
      (* LHS is empty. *)
      + assert (J40: fst s = []). rewrite <- HeqLHS ; auto.
         assert (J41: fst (nodupseq s) = []). unfold nodupseq ; simpl. apply (nodup_nil (fst s)) ; auto.
          pose (@GUI_inv_critic_not_init_emptyLHS p s _ _ J0 c J40 H0 J1). rewrite <- e ; clear e.
          pose (@GUI_inv_critic_not_init_emptyLHS p (nodupseq s) _ _ J00 H J41 H1 J10). rewrite <- e ; clear e.
          epose (OrR (_,_)). simpl in g. apply g ; clear g.
          epose (OrL (_,_)). simpl in g. apply g ; clear g.
          epose (list_disj_L _ (_,_)). apply g ; clear g. simpl ; intros.
          pose (restr_list_prop_nodup (snd s) A p). destruct p0. apply i in H2.
          epose (list_disj_wkn_R _ (_,_) _ H2). simpl in g. apply g ; clear g.
          epose (Id_all_form _ [] _ []). simpl in d ; apply d.
          pose (@GLS_prv_wkn_R (list_disj (map Box (map (UI p) (GLR_prems (nodupseq (nodupseq s))))) :: X, list_disj (map Box (map (UI p) (GLR_prems (nodupseq s)))) :: Y)).
          apply g with (A:=list_disj (restr_list_prop p (snd s))). clear g.
          ...
          apply InT_map_iff. exists (UI p x0). split ; auto. apply InT_map_iff. exists x0 ; split ; auto. rewrite fixpoint_nodupseq ; auto.
          epose (Id_all_form _ [] _ []). simpl in d ; apply d.
      (* LHS is not empty. *)
      + rewrite HeqLHS in *. assert (J40: fst s <> []). rewrite <- HeqLHS ; intro Hn ; inversion Hn. assert (J41: fst (nodupseq s) <> []).
         intro. apply J40. unfold nodupseq in *. simpl in H2. apply nodup_nil ; auto.
         pose (@GUI_inv_critic_not_init_not_emptyLHS p s _ _ _ J0 c J40 H0 J1 J2). rewrite <- e ; clear e.
         pose (@GUI_inv_critic_not_init_not_emptyLHS p (nodupseq s) _ _ _ J00 H J41 H1 J10 J20). rewrite <- e ; clear e.
    ...
    * assert ((is_init s) -> False) ; auto. assert ((is_init (nodupseq s)) -> False). intro. apply H0.
      ...
      + assert (J40: fst s = []). rewrite <- HeqLHS ; auto.
         assert (J41: fst (nodupseq s) = []). unfold nodupseq ; simpl. apply (nodup_nil (fst s)) ; auto.
          ...
          epose (OrL (_,_)). simpl in g. apply g ; clear g.
          epose (list_disj_L _ (_,_)). apply g ; clear g. simpl ; intros.
          pose (restr_list_prop_nodup (snd s) A p). destruct p0. apply i0 in H2.
          epose (list_disj_wkn_R _ (_,_) _ H2). simpl in g. apply g ; clear g.
          epose (Id_all_form _ [] _ []). simpl in d ; apply d.
          pose (@GLS_prv_wkn_R (list_disj (map Box (map (UI p) (GLR_prems (nodupseq s)))) :: X, list_disj (map Box (map (UI p) (GLR_prems (nodupseq (nodupseq s))))) :: Y)).
          apply g with (A:=list_disj (restr_list_prop p (snd (nodupseq s)))). clear g.
...
(* Can I try the other way around for the above lemma? If it holds, then the lowest lemma should go through easily. *)
