...
Require Import UIK_UI_prelims.
  ...
  - intros X0 Y0 hei idseq propvar. inversion k0 ; subst.
    (* IdP *)
    * inversion H ; subst.
      ...
      + destruct (critical_Seq_dec k).
         -- destruct (dec_KS_init_rules k).
            ...
            ** assert ((is_init k) -> False) ; auto. assert (J0: GUI p k (UI p k)). apply UI_GUI ; auto.
                ...
                apply UI_GUI ; auto.
                assert (fst k <> []). intro. rewrite H1 in i. inversion i.
                assert (J2: GUI p (unboxed_list (top_boxes (fst k)), []) (UI p (unboxed_list (top_boxes (fst k)), []))). apply UI_GUI ; auto.
                pose (@GUI_inv_critic_not_init_not_emptyLHS p k _ _ _ J0 c H1 H0 J1 J2). rewrite <- e.
      ...
      + destruct (critical_Seq_dec k).
         -- destruct (dec_KS_init_rules k).
            ...
            ** assert ((is_init k) -> False) ; auto. assert (J0: GUI p k (UI p k)). apply UI_GUI ; auto.
                ...
                ++ assert (J2: GUI p (unboxed_list (top_boxes (fst k)), []) (UI p (unboxed_list (top_boxes (fst k)), []))). apply UI_GUI ; auto.
                    assert (J4: fst k = []). auto.
                    pose (@GUI_critic_not_init_emptyLHS p k _ c H0 J4 J1). apply UI_GUI in g. rewrite g. clear g.
                    pose (OrR (X0,Y0)). simpl in k1. apply k1.
                    ...
                    apply In_list_In_propvar_subform_list ; apply InT_In ; auto.
                    pose (list_disj_wkn_R (restr_list_prop p (snd k)) (X0, list_disj (map Box (map (UI p) (KR_prems k))) :: Y0) (# P) J3). apply k2. simpl.
                    apply InT_split in i. destruct i. destruct s. rewrite e. apply derI with (ps:=[]). apply IdP. 2: apply dlNil.
                    assert ((x ++ # P :: x0, # P :: list_disj (map Box (map (UI p) (KR_prems k))) :: Y0) = (x ++ # P :: x0, [] ++ # P :: list_disj (map Box (map (UI p) (KR_prems k))) :: Y0)).
                    auto. rewrite H1 ; apply IdPRule_I.
                ++ assert (J2: GUI p (unboxed_list (top_boxes (fst k)), []) (UI p (unboxed_list (top_boxes (fst k)), []))). apply UI_GUI ; auto.
                    assert (J4 : fst k <> []). intro. rewrite H1 in HeqLHS. inversion HeqLHS.
                    pose (@GUI_inv_critic_not_init_not_emptyLHS p k _ _ _ J0 c J4 H0 J1 J2). rewrite <- e.
                    pose (OrR (X0,Y0)). simpl in k1. apply k1.
                    assert (J3: InT (# P)  (restr_list_prop p (snd k))). unfold restr_list_prop. apply In_InT.
                    apply in_not_touched_remove. apply In_list_In_list_prop_LF ; apply InT_In ; auto.
                    intro. apply propvar. rewrite <- H1. rewrite propvar_subform_list_app. apply in_or_app ; left.
    ...
    * destruct (critical_Seq_dec k).
      ...
      + inversion X ; subst.
        ...
        ** apply univ_gen_ext_splitR in X1. destruct X1. destruct s. destruct p0. destruct p0.
         ...
         remember (fst k) as LHS. destruct LHS.
         (* LHS is empty *)
         ++ assert (J80: fst k = []). auto.
             pose (@GUI_critic_not_init_emptyLHS p k _ c H J80 J1). apply UI_GUI in g. rewrite g ; clear g.
             repeat destruct s ; destruct p0 ; simpl ; subst.
             (* If Box A is in Y0. *)
             -- pose (OrR (X0,Box A :: Δ1)). simpl in k1. apply k1. clear k1. inversion u ; subst. simpl in x.
                apply KS_hpadm_wkn_R with (A:=list_disj (restr_list_prop p (snd k))) (s:=(X0,
                list_disj (map Box (map (UI p) (KR_prems k))) :: Box A :: Δ1)).
                2: epose (wkn_RI (list_disj (restr_list_prop p (snd k))) _ [] _) ; simpl in w ; apply w.
                apply KS_hpadm_wkn_R with (A:=list_disj (map Box (map (UI p) (KR_prems k)))) (s:=(X0, Box A :: Δ1)).
                2: epose (wkn_RI (list_disj (map Box (map (UI p) (KR_prems k)))) _ [] _) ; simpl in w ; apply w.
                apply derI with (ps:=[(unboxed_list x1, [A])]). apply KR. assert ((Box A :: Δ1) = [] ++ Box A :: Δ1). auto. rewrite H0.
                apply KRRule_I ; auto. apply dlCons. 2: apply dlNil. simpl in x. auto.
            -- destruct x2 ; simpl in e1 ; subst.
                (* If Box A is in Y0 (bis). *)
                +++ rewrite <- app_nil_end in e1 ; subst. simpl in e2 ; subst. inversion u ; subst. simpl in x.
               ...
               +++ inversion e1 ; subst.
                    ...
                    apply KS_hpadm_wkn_R with (A:=list_disj (restr_list_prop p (snd k))) (s:=(X0,
                    (list_disj (map Box (map (UI p) (KR_prems k)))) :: Y0)).
                    2: epose (wkn_RI (list_disj (restr_list_prop p (snd k))) _ [] _) ; simpl in w ; apply w.
                    pose (list_disj_wkn_R (map Box (map (UI p) (KR_prems k))) (X0, Y0)).
                    ...
                    inversion e2 ; subst. rewrite <- H2. destruct k ; auto. rewrite H0. apply KRRule_I ; auto.
                    intro. intros. apply H1 ; apply in_or_app ; auto. rewrite <- HeqLHS ; auto.
             (* If Box A is in Y0 (ter). *)
             -- pose (OrR (X0,x2 ++ Box A :: Δ1)). simpl in k1. apply k1. clear k1.
                apply KS_hpadm_wkn_R with (A:=list_disj (restr_list_prop p (snd k))) (s:=(X0,
                (list_disj (map Box (map (UI p) (KR_prems k)))) :: x2 ++ Box A :: Δ1)).
                2: epose (wkn_RI (list_disj (restr_list_prop p (snd k))) _ [] _) ; simpl in w ; apply w.
                apply KS_hpadm_wkn_R with (A:=list_disj (map Box (map (UI p) (KR_prems k)))) (s:=(X0, x2 ++ Box A :: Δ1)).
                2: epose (wkn_RI (list_disj (map Box (map (UI p) (KR_prems k)))) _ [] _) ; simpl in w ; apply w.
                apply derI with (ps:=[(unboxed_list x1, [A])]). apply KR. apply KRRule_I ; auto. intros B H2. apply H1. apply in_or_app ; auto.
                apply dlCons. 2: apply dlNil. inversion u ; subst ; simpl in x. auto.
         (* LHS is not empty *)
         ++ assert (J2: GUI p (unboxed_list (top_boxes (fst k)), []) (UI p (unboxed_list (top_boxes (fst k)), []))). apply UI_GUI ; auto.
             assert (J80: fst k <> []). intro. rewrite H0 in HeqLHS. inversion HeqLHS.
             pose (@GUI_inv_critic_not_init_not_emptyLHS p k _ _ _ J0 c J80 H J1 J2). rewrite <- e1. clear e1.
             repeat destruct s ; destruct p0 ; subst.
             (* If Box A is in Y0. *)
             -- pose (OrR (X0,Box A :: Δ1)). simpl in k1. apply k1. clear k1.
                apply KS_hpadm_wkn_R with (A:=list_disj (restr_list_prop p (snd k))) (s:=(X0,
                ...
                simpl. assert ((top_boxes (fst k)) = x0). symmetry. rewrite <- HeqLHS ; apply nobox_gen_ext_top_boxes_identity ; auto.
