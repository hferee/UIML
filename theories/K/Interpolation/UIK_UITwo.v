  ...
  - destruct (dec_KS_init_rules s).
    ...
    + remember (fst s) as LHS. destruct LHS.
      ...
      * assert (P2: KS_prv (list_disj (restr_list_prop p (snd s)) :: fst s, snd s)).
         ...
         assert (GUI p s
         (Or (list_disj (restr_list_prop p (snd s))) (list_disj (map Box (map (UI p) (KR_prems s)))))).
         apply GUI_critic_not_init_emptyLHS ; auto. apply Gimap_map ; intros. apply UI_GUI ; auto.
         apply (UI_GUI p s) in H0. rewrite H0. rewrite HeqLHS. repeat apply OrL ; auto.
      (* LHS s is not empty *)
      * assert (P1: KS_prv (list_disj (map Neg (restr_list_prop p (fst s))) :: fst s, snd s)).
        ...
        rewrite H. apply Id_all_form. apply derI with (ps:=[]). apply BotL. apply BotLRule_I. apply dlNil.
         ...
         apply DiamL_lim with (BΓ:=top_boxes (fst s)). apply is_Boxed_list_top_boxes. apply nobox_top_boxes.
         assert (J0: measure (unboxed_list (top_boxes (fst s)), []%list) < measure s).
         unfold measure. simpl. destruct s ; simpl ; subst. simpl in * ; subst ; simpl.
         destruct m ; simpl ; pose (size_unboxed (top_boxes LHS)) ; pose (size_top_boxes LHS) ; lia.
         assert (J30: measure (unboxed_list (top_boxes (fst s)), []%list) = measure (unboxed_list (top_boxes (fst s)), []%list)) ; auto.
         ...
         apply GUI_critic_not_init ; auto. intro. subst. rewrite H0 in HeqLHS ; inversion HeqLHS.
         apply Gimap_map ; intros. 1-2: apply UI_GUI ; auto.
         apply (UI_GUI p s) in H0. rewrite H0. rewrite HeqLHS. repeat apply OrL ; auto.
